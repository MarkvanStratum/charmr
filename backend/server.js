...
import {
  ensureImageColumns,
  isSexualExplicit,
  shouldSendSuggestiveImage,
  pickRandomImageUrl,
  insertImageMessage,
  insertAssistantTextMessage,
  pickSafeFlirtyLine
} from "./services/spicyMedia.js";
...
(async () => {
  ...
  reset_token_expires TIMESTAMP  -- expiration time for the reset token
    ...
    // ✅ ensure columns for image messages exist (no-op if already present)
  ...
  try {
    ...
    for (const msg of result.rows) {
      ...
      grouped[key].push({
        ...
        text: msg.text,
        time: msg.created_at,
        // ✅ added so the client can render images
        type: msg.type || "text",
...
if (!user.lifetime) {
  await pool.query("UPDATE users SET credits = credits - 1 WHERE id = $1", [userId]);
    ...
    // ✅ NEW: trigger image + playful line on sexual/suggestive messages
    if (isSexualExplicit(message) || shouldSendSuggestiveImage(message)) {
      const imageUrl = pickRandomImageUrl(girlId);
      if (imageUrl) {
        await insertImageMessage({ pool, userId, girlId, imageUrl });
        const line = pickSafeFlirtyLine();
        await insertAssistantTextMessage({ pool, userId, girlId, text: line });
      }
    }
