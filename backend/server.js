...
const personalities = {
  // your personalities here if needed
};
...
// NEW ROUTE: Send hardcoded initial message from a specified girl to user
app.post("/api/send-initial-message", (req, res) => {
  ...
  // Avoid duplicate initial messages: check if already sent
  const alreadySent = messages[chatKey].some(msg => msg.from === "assistant" && msg.text === firstMsg);
  if (alreadySent) {
    return res.json({ message: "Initial message already sent" });
  }

  conversations[userId][girlId].push({ role: "assistant", content: firstMsg });
  messages[chatKey].push({ from: "assistant", text: firstMsg });
